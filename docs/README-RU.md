...
- **Значок плана** - Отображает вашу текущую подписку (Pro, Max 5x, Max 20x, Team, Enterprise)
- **Недельные лимиты по моделям** - Отдельные строки для Fable, Opus и Sonnet, когда ваш план их сообщает
- **Кредиты использования** - Показывает разовые кредиты и срок их действия
- **Поддержка дополнительного использования** - Отслеживайте расходы с оплатой по мере использования и смотрите, почему оно отключено, если организация его выключила
...
- **Настраиваемые уведомления** - Нативные уведомления Центра уведомлений при 50%, 75%, 100% или при сбросе для каждого отслеживаемого лимита
...
Автоматически обходит проверку безопасности macOS Gatekeeper — дополнительных действий не требуется.
...
ClaudeBar читает учетные данные OAuth из связки ключей macOS, которые Claude Code сохраняет при входе. Затем он запрашивает тот же эндпоинт использования, что и команда `/usage` в Claude Code.

Когда срок токена подходит к концу, ClaudeBar обновляет его по тому же протоколу, что и Claude Code: сначала берёт блокировку обновления Claude Code (`~/.claude.lock`), перечитывает связку ключей и использует новый токен, если Claude Code уже обновил его; иначе обновляет сам и записывает на место. Запись передаётся инструменту `security` через stdin, поэтому токен никогда не появляется в аргументах процесса. Если связка ключей недоступна, используется `~/.claude/.credentials.json`.
...
Чтение и запись идут через системный инструмент `security`, тем же путём, что и Claude Code, поэтому дополнительный запрос связки ключей обычно не появляется. Если macOS всё же спросит, нажмите **Всегда разрешать**.

### Уведомления

При первом запуске macOS спросит, может ли ClaudeBar показывать уведомления. Если отказать, их можно включить позже в Системных настройках › Уведомления › ClaudeBar.

### Конфиденциальность

- Читает только учетные данные, уже сохранённые Claude Code, и записывает лишь обновлённые токены
