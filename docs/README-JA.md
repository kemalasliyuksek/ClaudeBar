...
- **プランバッジ** - 現在のサブスクリプションを表示（Pro、Max 5x、Max 20x、Team、Enterprise）
- **モデル別の週間制限** - プランが報告する場合、Fable、Opus、Sonnet を個別の行で表示
- **使用クレジット** - 一回限りのクレジットとその有効期限を表示
- **追加使用量サポート** - 従量課金の支出を追跡し、組織が無効にした場合はその理由を表示
- **カラーコード付きプログレスバー** - 使用率に応じて緑、黄、オレンジ、赤で表示
- **多言語対応** - 英語、トルコ語、中国語、スペイン語、ロシア語、アプリ内言語切替
- **カスタマイズ可能な通知** - 追跡中のすべての制限について、50%、75%、100%、またはリセット時にネイティブの通知センターで通知
...
ClaudeBar は、Claude Code がログイン時に保存する OAuth 認証情報を macOS キーチェーンから読み取ります。その後、Claude Code の `/usage` コマンドと同じ使用量エンドポイントに問い合わせます。

トークンの有効期限が近づくと、ClaudeBar は Claude Code と同じプロトコルで更新します。まず Claude Code の更新ロック（`~/.claude.lock`）を取得し、キーチェーンを読み直して、Claude Code が既に更新していればその新しいトークンを使います。そうでなければ自身で更新し、その場で書き換えます。書き込みは `security` ツールに stdin 経由で渡されるため、トークンがプロセス引数に現れることはありません。キーチェーンが利用できない場合は `~/.claude/.credentials.json` にフォールバックします。
...
読み書きはシステムの `security` ツールを経由し、Claude Code と同じ経路を使うため、通常は追加のキーチェーン確認は表示されません。macOS が確認を求めた場合は **常に許可** をクリックしてください。

### 通知

初回起動時、macOS は ClaudeBar に通知の表示を許可するか尋ねます。拒否した場合は、後から システム設定 › 通知 › ClaudeBar で有効にできます。

### プライバシー

- Claude Code が既に保存した認証情報のみを読み取り、書き戻すのは更新後のトークンだけです
- すべての通信はHTTPSを使用
