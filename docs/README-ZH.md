...
- **订阅计划徽章** - 显示您当前的订阅类型（Pro、Max 5x、Max 20x、Team、Enterprise）
- **按模型的每周限额** - 当您的计划报告时，为 Fable、Opus 和 Sonnet 分别显示一行
- **使用额度** - 显示一次性额度及其到期时间
- **额外用量支持** - 追踪按需付费支出，并在组织关闭时查看原因
- **彩色进度条** - 根据使用百分比显示绿色、黄色、橙色、红色
- **多语言支持** - 英语、土耳其语、中文、西班牙语、俄语，支持应用内语言切换
- **可自定义通知** - 针对每个受监控的限额，在 50%、75%、100% 或重置时通过原生通知中心提醒
...
ClaudeBar 从 macOS 钥匙串中读取 Claude Code 登录时存储的 OAuth 凭据，然后查询与 Claude Code 的 `/usage` 命令相同的用量端点。

当令牌即将过期时，ClaudeBar 使用与 Claude Code 相同的协议刷新它：先获取 Claude Code 的刷新锁（`~/.claude.lock`），重新读取钥匙串，如果 Claude Code 已经刷新则直接使用新令牌；否则自行刷新并就地更新。写入通过 stdin 传递给 `security` 工具，因此令牌不会出现在进程参数中。如果钥匙串不可用，则回退到 `~/.claude/.credentials.json`。
...
读写都通过系统的 `security` 工具进行，与 Claude Code 使用同一路径，因此通常不会出现额外的钥匙串提示。如果 macOS 仍然询问，请点击 **始终允许**。

### 通知

首次启动时，macOS 会询问是否允许 ClaudeBar 显示通知。如果拒绝，之后可以在 系统设置 › 通知 › ClaudeBar 中重新开启。

### 隐私

- 仅读取 Claude Code 已存储的凭据，仅写回刷新后的令牌
- 所有通信使用 HTTPS
- 不在系统钥匙串之外存储任何数据
- 无数据分析或遥测
- 完全开源
