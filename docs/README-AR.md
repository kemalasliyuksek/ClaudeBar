...
- **شارة الخطة** - يعرض اشتراكك الحالي (Pro، Max 5x، Max 20x، Team، Enterprise)
- **حدود أسبوعية لكل نموذج** - صفوف منفصلة لـ Fable و Opus و Sonnet عندما تبلغ عنها خطتك
- **أرصدة الاستخدام** - يعرض الأرصدة لمرة واحدة وتاريخ انتهائها
- **دعم الاستخدام الإضافي** - تتبع إنفاق الدفع حسب الاستخدام، واعرف سبب تعطيله عندما توقفه مؤسستك
...
- **إشعارات قابلة للتخصيص** - تنبيهات أصلية من مركز الإشعارات عند 50%، 75%، 100% أو عند إعادة التعيين لكل حد يتم تتبعه
...
يقرأ ClaudeBar بيانات اعتماد OAuth من سلسلة مفاتيح macOS التي يخزنها Claude Code عند تسجيل الدخول. ثم يستعلم من نقطة نهاية الاستخدام نفسها التي يستخدمها أمر `/usage` في Claude Code.

عندما يقترب الرمز من الانتهاء، يجدده ClaudeBar بنفس بروتوكول Claude Code: يأخذ أولاً قفل التجديد الخاص بـ Claude Code (`~/.claude.lock`)، ويعيد قراءة سلسلة المفاتيح ويستخدم الرمز الجديد إذا كان Claude Code قد جدده بالفعل؛ وإلا يجدده بنفسه ويحدّثه في مكانه. تُمرَّر الكتابة إلى أداة `security` عبر stdin، لذا لا يظهر الرمز أبداً في وسائط العملية. إذا لم تكن سلسلة المفاتيح متاحة، يُستخدم `~/.claude/.credentials.json` كبديل.
...
تمر عمليات القراءة والكتابة عبر أداة `security` في النظام، وهو المسار نفسه الذي يستخدمه Claude Code، لذا لا يظهر عادةً أي طلب إضافي من سلسلة المفاتيح. إذا سأل macOS، انقر على **السماح دائماً**.

### الإشعارات

عند التشغيل الأول، يسأل macOS عما إذا كان بإمكان ClaudeBar عرض الإشعارات. إذا رفضت، يمكنك تفعيلها لاحقاً من إعدادات النظام › الإشعارات › ClaudeBar.

### الخصوصية

- يقرأ فقط بيانات الاعتماد التي خزنها Claude Code بالفعل، ولا يكتب سوى الرموز المجددة
